...
const chatSlice = createSlice( {
    ...
    initialState,
    reducers: {
        setMessagesForContact: ( state, action: PayloadAction<{ contactId: string; messages: ChatMessage[] }> ) =>
        {
            state[action.payload.contactId] = [...action.payload.messages];
        },

        addMessageToContact: ( state, action: PayloadAction<{ contactId: string; message: ChatMessage }> ) =>
        {
            const { contactId, message } = action.payload;
            const exists = ( state[contactId] || [] ).some( msg => msg.id && message.id && msg.id === message.id );
            if ( !exists ) state[contactId] = [...( state[contactId] || [] ), message];
        },

        deleteMessageForContact: ( state, action: PayloadAction<{ contactId: string; index: number }> ) =>
        {
        ...
        clearMessagesForContact: ( state, action: PayloadAction<{ contactId: string }> ) =>
        {
            state[action.payload.contactId] = [];
        },

        updateMessageForContact: ( state, action: PayloadAction<{
            contactId: string;
            messageId: string;
            newText?: string;
            newCaption?: string;
        ...
        removeMessageById: ( state, action: PayloadAction<{ contactId: string; messageId: string }> ) =>
        {
...
export const addMessageAndSync = ( payload: { contactId: string; message: ChatMessage } ) => ( dispatch: any ) =>
{
    dispatch( chatSlice.actions.addMessageToContact( payload ) );
    dispatch(
        upsertLastMessage( {
            chat_partner_id: payload.contactId,
            message_id: payload.message.id || Date.now().toString(),
...
export const updateMessageAndSync = ( payload: {
    contactId: string;
    messageId: string;
    newText?: string;
    isDeleted?: boolean;
